<html>
<head>
<title>Eliminar</title>
</head>
<body bgcolor="#003366">
<center>

  <form method="POST" action="EliminarObjetivo.asp">
  <p style="color:white" > <font face="Verdana" size="5">Eliminar<p>
  <table border="1" cellspacing="1" bordercolor="#111111" id="AutoNumber1" height="82">

    <tr>
      <td height="23"><font color="#FFFFFF" face="Verdana" size="2">Numero de Cuenta</font></td>
      <td width="148" height="23">
        <p align="center">
        <font face="Verdana" color="#FFFFFF">
        <input type="text" name="numCuenta" size="20" value=""><font size="2">
        </font></font>
      </td>
    <tr>

    

      <td width="210" colspan="2" height="30">
      <p align="center"><font face="Verdana" color="#FFFFFF"><input type="submit" value="Eliminar" name="edit"><font size="2">&nbsp;</font></td>
    </tr>
  </table>
  </form>
  </center>
</body>
</html>